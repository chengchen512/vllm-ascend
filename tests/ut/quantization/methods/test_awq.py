...
def test_awq_pack_matches_reference(
    ...
    _require_npu_triton()
    from vllm_ascend.ops.triton.awq_conversion import awq_direct_pack, awq_pack_zero_triton
...
def test_ascend_fused_moe_reads_awq_quant_type():
    moe_config = MagicMock()
    config = AscendAWQConfig.from_config({"bits": 4, "group_size": 128, "zero_point": True})
    method = AscendAWQFusedMoEMethod(config, moe_config)
    ...
    assert layer._get_quant_type() == QuantType.W4A16


def test_awq_moe_apply_reads_layer_runtime_state():
    moe_config = MagicMock()
    config = AscendAWQConfig.from_config({"bits": 4, "group_size": 128, "zero_point": True})
    method = AscendAWQFusedMoEMethod(config, moe_config)
    layer = torch.nn.Module()
    layer.tid2eid = torch.ones(1, 1, dtype=torch.int32)
    layer.dynamic_eplb = True
    routed_out = torch.empty(1, 1)

    with patch.object(method, "_apply_ascend_moe", return_value=routed_out) as mock_apply:
        output = method.apply(
            layer,
            torch.empty(1, 1),
            torch.empty(1, 1),
    ...
    assert mock_apply.call_args.kwargs["tid2eid"] is layer.tid2eid
    assert mock_apply.call_args.kwargs["dynamic_eplb"] is layer.dynamic_eplb


def test_awq_moe_apply_adapts_to_v018_native_w4a16():
    moe_config = MagicMock()
    config = AscendAWQConfig.from_config({"bits": 4, "group_size": 128, "zero_point": True})
    method = AscendAWQFusedMoEMethod(config, moe_config)
    layer = torch.nn.Module()
    ...
    with patch(
        "vllm_ascend.quantization.methods.w4a16.AscendW4A16FusedMoEMethod.apply",
        autospec=True,
        return_value=routed_out,
    ) as mock_apply:
        output = method._apply_ascend_moe(
            layer,
            hidden_states,
            router_logits,
            top_k=2,
            renormalize=True,
            num_experts=4,
            tid2eid=torch.ones(1, 1, dtype=torch.int32),
            input_ids=torch.ones(1, dtype=torch.int64),
            dynamic_eplb=True,
        )

    assert output is routed_out
    native_method = mock_apply.call_args.args[0]
    assert native_method.dynamic_eplb is True
    assert mock_apply.call_args.args[1:4] == (
        layer,
        hidden_states,
        router_logits,
    )
    assert mock_apply.call_args.kwargs["global_num_experts"] == 4
    assert "num_experts" not in mock_apply.call_args.kwargs
    assert "tid2eid" not in mock_apply.call_args.kwargs
    assert "input_ids" not in mock_apply.call_args.kwargs
...
def test_awq_config_dispatch():
    ...
    default_config = AscendAWQConfig.from_config({"bits": 4, "group_size": 128, "zero_point": True})
    assert config.lm_head_quantized
    assert not default_config.lm_head_quantized
    ...
    quantized_moe = config.get_quant_method(_new_routed_experts_layer(), "model.layers.0.mlp.experts", tid2eid={})
    quantized_lm_head = config.get_quant_method(_new_lm_head_layer(), "lm_head")
    skipped_lm_head = default_config.get_quant_method(_new_lm_head_layer(), "lm_head")
    ...
    assert quantized_moe.supports_eplb
